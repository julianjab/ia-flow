...
- **Deploy headless:** `agents/<deploy>/agents.*.yaml` cargado por `YamlAgentRepository`
  (read-only, sin CRUD en runtime) junto a `projects.yaml`, `repos.yaml` y `mcp-catalog.yaml`.
