...
// Las tools de Slack — las ve un agente que las nombre en su `tools[]`.
//
// A diferencia del resto de `@ia-flow/tools`, acá el registro NO es un efecto
// de importar: es `registerSlackTools()`, que `installSlack` llama sólo cuando
// hay `SLACK_BOT_TOKEN`. La razón es lo que ve el operador: el editor de
// agentes lista `GET /api/tools`, y una `slack_post_message` ofrecida por un
// proceso sin credencial es una tool que se puede tildar y siempre falla.
// `unregisterSlackTools()` es la contracara, para que pegar o borrar el token
// desde Configuración se note sin reiniciar.
...
/** Los nombres que este paquete registra. `unregisterSlackTools` los necesita,
 *  y tenerlos en una constante evita que sacar Slack se olvide de una. */
...
/** Registra las cuatro tools en el registry del proceso. Idempotente:
 *  `registerTool` pisa por nombre. */
export function registerSlackTools(): void {
  ...
  registerTool({
    ...
    description:
      'Read all replies of a Slack thread given the channel ID and the parent message ts.',
    input_schema: {
      type: 'object',
      properties: {
  ...
  registerTool({
    ...
    input_schema: {
      ...
      properties: {
        channel: {
          type: 'string',
          description: 'Channel ID (C...), DM ID (D...), or #channel-name',
        },
