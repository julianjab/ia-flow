...
| Sin sesión, `getToken()` devuelve `undefined` (no tira) | Nadie corrió el login todavía = integración no configurada, que es el estado normal de quien no usa Figma. Un throw ahí rompería runs que no tienen nada que ver. Ojo con qué significa `undefined` río abajo: `interpolateMcpServers` hace `(await resolveSecret(name)) ?? ''`, así que el MCP **sí** queda configurado, con `authorizationToken` vacío, y Figma contesta 401. Es lo correcto —fallar en la API antes que filtrar el placeholder crudo— pero el síntoma es un 401, no un MCP ausente. |
