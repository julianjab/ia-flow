...
export interface ProviderInput {
  ...
  projectId?: string
  /** Profundidad de delegación de este run (0 = agente de más arriba). Viaja
   *  al `ToolContext` para que `run_agent` pueda frenar una cadena circular:
  ...
  runId?: string
  /** The webhook delivery/scan that started the whole cascade (event → rule
   *  → this run → any sub-agent it spawns) — spans multiple runIds, unlike
   *  `runId` which only ties together ONE run's log lines. Purely for
   *  correlation: a provider stamps it into its log lines the same way it
   *  already does with `runId`, nothing reads it to decide behavior. */
  traceId?: string
  taskId: string
  taskTitle: string
