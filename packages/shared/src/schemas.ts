...
// ─── Disposición de una tarea (GET /api/tasks/dispositions) ───────────────
//
// La pregunta que ordena la app no es "¿en qué estado está esto?" sino
// **"¿quién tiene que mover la próxima pieza?"**. Con agentes trabajando solos
// las dos dejaron de coincidir: lo más reciente es casi siempre lo que avanza
// sin vos, y lo que te espera es justamente lo que lleva más tiempo quieto — o
// sea, lo que un orden por fecha manda al fondo.
//
// **Se decide en el SERVER y no en el cliente**, y no es una preferencia: la
// disposición depende de si existe una regla de retry que vaya a matchear este
// fallo, de los blockers que resuelve la fuente, y de la cuota del agente. El
// cliente no conoce ninguna de las tres. Calcularlo en el browser obligaba a
// bajar las reglas y re-implementar el matcher, y aun así daría distinto que
// el engine — que es la peor forma de estar mal: parecer que se sabe.

export const TaskDispositionSchema = z.enum([
  /** Nada va a pasar hasta que vos hagas algo. No hay agente ni regla que lo
   *  tome. */
  'waiting-on-you',
  /** Hay un agente corriendo, o una regla que va a disparar. Lo mirás; no lo
   *  tocás. */
  'moving',
  /** Espera a otro ítem de la app, no a vos. La acción real está en el
   *  bloqueante. */
  'blocked',
  /** Terminó y el estado ya avanzó. Nadie espera nada. */
  'closed',
])
export type TaskDisposition = z.infer<typeof TaskDispositionSchema>

/**
 * A dónde lleva el verbo de una fila del bucket 1.
 *
 * **Ninguna fila inventa una capacidad.** El verbo es un DESTINO: si la app
 * puede ejecutarlo, llama a su endpoint; si no, navega a la pantalla —propia o
 * de GitHub— donde eso se hace hoy. El texto nombra el destino, no la
 * intención, y por eso lo resuelve el server: es quien sabe si hay un run que
 * reintentar, una ejecución que abortar o un PR abierto al que ir.
 */
export const TaskVerbSchema = z.object({
  /** Lo que se lee en la fila: `Reintentar`, `Revisar el PR`. */
  label: z.string(),
  kind: z.enum([
    /** POST /api/tasks/:id/run */
    'run',
    /** POST /api/executions/:id/cancel — trae `executionId`. */
    'cancel',
    /** Una pantalla de la app; `href` es la ruta del router. */
    'route',
    /** Fuera de la app (hoy, sólo el PR en GitHub); `href` es absoluta y abre
     *  en una pestaña nueva. */
    'external',
  ]),
  href: z.string().optional(),
  executionId: z.string().optional(),
  /** Se dibuja atenuado detrás del label: `↗ github`, `· runs abortados`.
   *  Decir a dónde lleva es lo que evita que un verbo prometa de más. */
  hint: z.string().optional(),
})
export type TaskVerb = z.infer<typeof TaskVerbSchema>

export const TaskDispositionEntrySchema = z.object({
  taskId: z.string(),
  disposition: TaskDispositionSchema,
  /**
   * Por qué está en ese bucket, literal (O1).
   *
   * Una fila nunca dice sólo su estado: dice por qué te toca a vos.
   * `falló · tests` describe; `falló 2× · no hay regla de retry` te dice que
   * sos vos. Es la información más valiosa que el server ya tiene y la UI no
   * mostraba.
   */
  reason: z.string(),
  /**
   * Desde cuándo quedó en TUS manos — no la fecha del último evento (O3).
   *
   * Son dos números distintos: una tarea ignorada seis días tiene su último
   * evento hace seis días, pero lleva seis días esperándote, y eso es lo que
   * hay que decir. `null` fuera del bucket 1, donde no significa nada.
   */
  waitingOnYouSince: z.string().nullable(),
  /** Cuántos ítems dependen de éste. Es el primer desempate del bucket 1: un
   *  PR que traba cuatro tareas vale más que un fallo aislado. `0` cuando la
   *  fuente no modela dependencias — no es "no traba nada", es que no se sabe,
   *  y como desempate se comporta igual. */
  unblocks: z.number().int().nonnegative(),
  /** Quién bloquea a ésta, para que la fila del bucket 3 sea navegable (O5). */
  blockedBy: z.array(z.string()).default([]),
  /** El verbo, sólo en `waiting-on-you`: en los otros tres ofrecer un botón es
   *  ruido, porque no te toca (O2). */
  verb: TaskVerbSchema.nullable().default(null),
})
export type TaskDispositionEntry = z.infer<typeof TaskDispositionEntrySchema>
export const TaskDispositionEntryArraySchema = z.array(TaskDispositionEntrySchema)

/** El orden de los buckets, y su nombre. Vive acá porque lo dibujan las tres
 *  vistas que son recortes del MISMO orden — Tareas, Qué sigue y Board (O6). */
export const DISPOSITION_ORDER: TaskDisposition[] = [
  'waiting-on-you',
  'moving',
  'blocked',
  'closed',
]
export const DISPOSITION_LABELS: Record<TaskDisposition, string> = {
  'waiting-on-you': 'te espera',
  moving: 'avanza solo',
  blocked: 'trabado por otro',
  closed: 'cerrado',
}
...
export const ServerLogFiltersSchema = z.object({
  ...
  // Cada entrada es `"<clave>:<patrón>"` (ej. `"taskId:abc*"`) para acotar a
  // un campo, o sólo `"<patrón>"` (ej. `"ECONNRESET"`) para probarlo contra
  // CUALQUIER campo de `extras` sin tener que saber de antemano en cuál
  // vive el dato. `*` (cualquier secuencia) y `?` (un carácter) son los
  // únicos comodines — NO es una regexp arbitraria: corre en el event loop
  // del daemon sobre cada línea de un log que puede tener decenas de miles,
  // y Bun no expone un motor de regex con timeout, así que un patrón
  // adversarial con backtracking catastrófico colgaría el proceso entero
  // (ver `globMatchFull` en `server-logs.ts`). A diferencia de los filtros
  // de arriba (membership exacto contra una lista de valores conocidos),
  // esto sirve para un campo o un patrón que no está en ningún selector —
  // el mismo rol que `search` cumple para `msg`. Multi-select: varias
