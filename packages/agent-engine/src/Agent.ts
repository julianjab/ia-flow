...
/**
 * Un run que reanuda una pausa entra con la conversación que el checkpoint
 * guardó, no con el prompt — pero el `brief` SÍ tiene que viajar, apendeado
 * como el próximo user turn: es la única forma en que un agente pausado con
 * `pause_until` se entera de qué pasó. Sin esto, despierta a la MISMA
 * conversación de antes de pausar (el fallback `resumeMessages ?? [prompt]`
 * del provider descarta el `prompt` —y con él el brief— entero en cuanto hay
 * checkpoint), sin ningún indicio de que el evento que esperaba ya llegó.
 */
function appendBriefToResumeMessages(
  resumeMessages: unknown[] | undefined,
  briefBlock: string | undefined,
): unknown[] | undefined {
  if (!resumeMessages) return undefined
  if (!briefBlock) return resumeMessages
  return [
    ...(resumeMessages as Array<{ role: string; content: unknown }>),
...
/**
 * Snapshot of the run-scoped state that `run()`'s extracted branch handlers
 * need — bundled so each handler takes one param instead of a dozen
 * positional ones. Built fresh at each call site (once before the
 * tmux/sync dispatch, once inside the `catch`) from variables that don't
 * change again after that point in `run()`, plus the current `task`.
 */
interface RunFrame {
  ...
  readonly exits: Record<string, AgentExit> | undefined
  readonly agentPromptHash: string | undefined
...
export class Agent {
   ...
   * run: un fallo acá (DB transitoria, JSON corrupto de una fila vieja) deja
   * `{{task.previous_outputs}}` vacío en vez de tumbar el dispatch.
   */
  private async loadPreviousOutputs(
    taskId: string,
  ): Promise<NonNullable<ResolveContext['previousOutputs']>> {
    try {
      return this.executionLogRepo?.listLastOutputsByAgent(taskId) ?? []
    } catch (err) {
      log.warn({ err, taskId }, 'Failed to load previous structured outputs')
      return []
    }
  }
   ...
   * (el resto sale gratis de `task.pullRequests`) — por eso sólo se pide
   * cuando el prompt la referencia, mismo mecanismo que gatea marcar
   * `{{task.comments}}` como leídos en `run()`. Sin PR abierto o sin
   * coordenadas de GitHub del repo primario, la variable rinde vacío en vez
   * de romper el render.
   */
  private async resolvePrDiff(
    agentDef: AgentDefinition,
    task: Task,
    projectRepos: RunContext['projectRepos'],
    primaryRepoName: string | undefined,
  ): Promise<string | undefined> {
    if (!promptReferencesVariable(agentDef.prompt, 'task.pr.diff')) return undefined
    const openPr = openPullRequests(task.pullRequests)[0]
    const primaryRepo = projectRepos.find((r) => r.name === primaryRepoName)
    if (!openPr || !primaryRepo?.githubOwner || !primaryRepo?.githubRepo) return undefined
    try {
      return await this.fetchPrDiff({
  ...
  /**
   * Resuelve el terreno concreto para este run: el engine declara la
   * INTENCIÓN (`workspaceRequest` — qué repos, qué branch, si el agente
   * escribe) y el provider devuelve el plan (worktree, checkout in-place o
   * nada). Arma también el bloque de git context que se antepone al prompt.
   * La branch que el provider terminó usando se refleja de vuelta en la
   * `task` devuelta.
   */
  private async prepareRunWorkspace(params: {
    agentDef: AgentDefinition
    task: Task
    runId: string
    projectRepos: RunContext['projectRepos']
    ...
    primaryRepoName: string | undefined
    primaryWorkflow: RunContext['primaryWorkflow']
  ...
  /**
   * Arma la identidad de este run: acota la ventana de comentarios, dispara
   * `onStart`, y decide si retoma la fila de `execution_logs` de un run sync
   * interrumpido (`reuseRow`) o abre una nueva.
   */
  private async initializeRunIdentity(
    input: AgentRunInput,
    ...
    // que markCommentsUsed vive en `run()` en vez de en el dispatcher, y lo
    // que hace que se marque exactamente lo que este run leyó.
    ...
    return { task, initialStatus, reuseRow, runId, logId, registryKey }
  }

  /**
   * Inserta o retoma la fila de `execution_logs` de este run. Una fila
   * reanudada (`reuseRow`) sólo actualiza los hashes — su contexto original
   * (`startedAt`, `ruleId`/`eventId`/..., `initialStatus`, `exits`,
   * `assignees`) queda como lo dejó el primer intento, porque no es un run
   * nuevo "que retoma otro": es el mismo, interrumpido a mitad de una vuelta.
   */
  private persistStartLog(params: {
    reuseRow: boolean
    logId: string
    agentPromptHash: string | undefined
    ...
    resolvedProviderId: string
    initialStatus: string
    exits: Record<string, AgentExit> | undefined
    input: AgentRunInput
  }): void {
    const {
      reuseRow,
  ...
  /**
   * Arma el `ProviderInput` completo para `provider.run()`. Los closures
   * (`drainMessages`, `saveCheckpoint`, …) sólo capturan `task.id` /
   * `task.projectId` — inmutables durante el resto del run — así que
   * recibir `task` por valor acá no cambia su comportamiento respecto de
   * capturarlo directo del scope de `run()`.
   */
  private async buildProviderInput(params: {
    agentDef: AgentDefinition
    task: Task
    input: AgentRunInput
    runId: string
    ...
    finalPrompt: string
    resumeMessages: unknown[] | undefined
    systemPromptBlocks: ReturnType<typeof resolveSystemPromptBlocks>
    ...
    return {
      ...
      prompt: finalPrompt,
      // Ver el comentario sobre `resumeMessages` en `run()`: el prompt igual
      // viaja completo — lo usan los providers de terminal, que no tienen
      // checkpoint.
      resumeMessages,
      systemPromptBlocks,
  ...
  /**
   * Marca como leídos los comentarios humanos que este run recibió, para que
   * no se re-inyecten en `{{task.comments}}` en un futuro re-dispatch de la
   * misma task (build → review → build tras un `fail_task`). Deliberadamente
   * llamado DESPUÉS de que `provider.run()` resuelve — para sync y async eso
   * significa que el modelo genuinamente recibió `finalPrompt` (con los
   * comentarios ya renderizados), no sólo que `TaskDispatcher` los cargó.
   */
  private async markCommentsUsedIfReferenced(
    agentDef: AgentDefinition,
    task: Task,
    manager: ITaskSource,
  ): Promise<void> {
    // TaskCommentSchema.id es opcional (una fuente sin markCommentsUsed nunca
    // lo puebla) — nos quedamos sólo con los que una fuente puede marcar.
  ...
  async run(input: AgentRunInput, runState: AgentRunState): Promise<Task> {
    ...
    const {
      ...
      logId,
      registryKey,
    } = await this.initializeRunIdentity(input, runState, lifecycle)
    let task = identifiedTask
    ...
    try {
      ...
      const previousOutputs = await this.loadPreviousOutputs(task.id)
      ...
      const prDiff = await this.resolvePrDiff(agentDef, task, projectRepos, primaryRepoName)
      ...
      // ── Workspace ────────────────────────────────────────────────────
      const workspace = await this.prepareRunWorkspace({
        agentDef,
        task,
        runId,
        projectRepos,
      ...
      const resumeMessages = appendBriefToResumeMessages(
      ...
      await this.markCommentsUsedIfReferenced(agentDef, task, manager)
      ...
      task =
        output.mode === 'tmux'
          ? await this.handleTmuxRun(frame, output, declaresOutput, missingOutput)
  ...
  /**
   * Registra el session handle de una sesión tmux/iterm: persiste sus
   * coordenadas para que la fila de `execution_logs` se pueda buscar /
   * cancelar después, registra `close()` como `killSession`, y arranca un
   * watchdog de liveness que finaliza el run si el usuario cierra la
   * pestaña a mano. `getTask` deja que el watchdog —que dispara en
   * cualquier momento hasta que `handleTmuxRun` lo desarma— siempre lea el
   * `task` más reciente en vez de una foto tomada al registrar.
   */
  private wireTmuxSession(
    frame: RunFrame,
    output: ProviderOutput,
    getTask: () => Task,
  ): { sessionHandle: SessionHandle | undefined; unwatchSessionLocal: (() => void) | undefined } {
    const { registryKey, logId, manager } = frame
    if (!output.session) return { sessionHandle: undefined, unwatchSessionLocal: undefined }
    ...
    return { sessionHandle: handle, unwatchSessionLocal }
  }

  /**
   * Rama async (`output.mode === 'tmux'`): registra el session handle, arma
   * el watchdog de liveness, y bloquea en `waitForFinish` hasta que la
   * propia sesión cierra el run vía `complete_task`/`fail_task`/cancel.
   */
  private async handleTmuxRun(
    frame: RunFrame,
    output: ProviderOutput,
    declaresOutput: boolean,
    missingOutput: () => Error,
  ): Promise<Task> {
    const { runState, agentDef, manager, registryKey, logId, runId, startedAtMs, toolsAvailable } =
      frame
    const { agentPromptHash, systemPromptHash } = frame
    let task = frame.task
    ...
    const { sessionHandle, unwatchSessionLocal } = this.wireTmuxSession(frame, output, () => task)
    log.info(
  ...
  /**
   * Rama sync (`output.mode === 'api'`): recoge lo que las tool calls
   * in-process ya mutaron sobre la task, corre `verify` si el agente lo
   * declara, y aplica la salida que corresponda.
   */
  private async handleSyncRun(
    frame: RunFrame,
    output: ProviderOutput,
    declaresOutput: boolean,
    missingOutput: () => Error,
    ...
    return output.truncated
      ? await this.handleTruncatedSyncRun(settledFrame, output)
      : await this.finalizeSuccessfulSyncRun(settledFrame, output, chosenExit)
  }

  /**
   * Una tool ya movió la task mientras corría el loop (o cerró el run ella
   * misma, `finalizedByTool`) — respeta esa decisión en vez de aplicar la
   * salida por default, que la pisaría.
   */
  private async finalizeMovedByToolSyncRun(
    frame: RunFrame,
    output: ProviderOutput,
  ...
  /**
   * `output.truncated`: pausa recuperable (budget/iteraciones agotados). No
   * corre la salida de éxito — postea un aviso de progreso y aplica la
   * salida de error para que el usuario pueda reintentar.
   */
  private async handleTruncatedSyncRun(frame: RunFrame, output: ProviderOutput): Promise<Task> {
    const {
      runState,
      agentDef,
      manager,
    ...
    // A `refusal` isn't recoverable by resuming the same conversation (ver
    // el aviso de abajo), así que no vale la pena conservar su checkpoint —
    // el orquestador lo borraría igual en su `finally`.
  ...
  /**
   * Camino de éxito sync: corre el gate de `verify` (si el agente lo
   * declara), postea el resumen del modelo como comentario de cierre, y
   * aplica la salida de éxito. `exits` puede ser `undefined` acá — un
   * agente clasificador (`tools: []`, sin `onProcess`/`onFinish`) no
   * declara ninguna a propósito, porque no mueve el board;
   * `applySuccessOutcome`/`resolveExit` (run-outcome.ts) ya son no-op
   * cuando `exits` falta, así que `lifecycle.end` simplemente no
   * transiciona nada — necesario igual para que la fila de
   * `execution_logs` cierre con `finishedAt`/`outcome` (reproducido en
   * vivo con `comment-triage`).
   */
  private async finalizeSuccessfulSyncRun(
    frame: RunFrame,
    output: ProviderOutput,
    chosenExit: string | undefined,
  ): Promise<Task> {
    const {
      runState,
      agentDef,
    ...
    // `handleGenericError` trata como cualquier otro fallo del run
    // (postError + lifecycle.fail); `classifyFailure` lo reconoce por esa
    // marca y le asigna `failureClass: 'verify_failed'`.
    //
    ...
    // después de que `verify` termina. Soltarlo antes dejaba entrar un
    // segundo dispatch del mismo agente/proyecto/provider mientras éste
    // todavía corría `bun test`/`typecheck` en el mismo worktree.
    //
    ...
    // ciclo lo reconozca cuando `handleGenericError` suelte el slot.
  ...
  /**
   * Clasifica por qué `run()` cayó al `catch` y despacha al handler que
   * corresponde: diferido por capacidad, cancelado explícitamente, abort de
   * upstream, o un fallo genuino. Mismo criterio que tenía el `catch`
   * inline antes de esta extracción — sólo cambia de función.
   */
  private async handleRunError(
    ...
    if (atCapacity) {
      return await this.handleAtCapacity(frame, err as ProviderAtCapacityError, errMsg)
    }
    if (explicitlyCancelled) {
      return this.handleExplicitCancel(frame)
    }
    if (upstreamAbort) {
      return await this.handleUpstreamAbort(frame, errMsg)
    }
    return await this.handleGenericError(frame, err, errMsg)
  }

  /**
   * El provider quedó al tope entre la sonda de admisión y el run (ver
   * `ProviderAtCapacityError`) — no es un fallo: nada del trabajo se
   * intentó, así que se difiere en vez de correr la salida de error del
   * agente.
   */
  private async handleAtCapacity(
    frame: RunFrame,
    err: ProviderAtCapacityError,
    errMsg: string,
  ): Promise<Task> {
    const { runState, agentDef, manager, runId, logId, startedAtMs, toolsAvailable } = frame
    const { agentPromptHash, systemPromptHash } = frame
    let task = frame.task
  ...
  /**
   * La divergencia de status (o un shutdown ordenado) canceló el run desde
   * afuera — `entry.cancel()` ya lo marcó antes de que este catch corriera.
   */
  private handleExplicitCancel(frame: RunFrame): Task {
    const { runState, agentDef, runId, logId, startedAtMs, toolsAvailable } = frame
    const { agentPromptHash, systemPromptHash } = frame
    const task = frame.task
    log.info(
      {
  ...
  /**
   * El fetch del provider murió solo (stream stall / red) sin que nadie lo
   * cancelara — se registra para el retry automático de `abortRepo`, sin
   * comentar nada en la task (el comentario de un fallo de infra no le
   * sirve a nadie: no es accionable por el usuario).
   */
  private async handleUpstreamAbort(frame: RunFrame, errMsg: string): Promise<Task> {
    const { runState, agentDef, manager, runId, logId, startedAtMs, toolsAvailable } = frame
    const { agentPromptHash, systemPromptHash } = frame
    let task = frame.task
    log.warn(
  ...
  /**
   * Un fallo genuino del run (no cancelado, no diferido, no un abort de
   * upstream). Si una tool ya movió la task antes de que el error surgiera,
   * respeta ese destino en vez de reaplicar la salida de error encima — el
   * resto aplica la salida de error del agente y siempre relanza `err` para
   * que la cadena del orquestador se corte, igual que antes de esta
   * extracción.
   */
  private async handleGenericError(frame: RunFrame, err: unknown, errMsg: string): Promise<never> {
    const {
      runState,
      agentDef,
      manager,
