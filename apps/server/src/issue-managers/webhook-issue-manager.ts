...
export const DEFAULT_WEBHOOK_DEBOUNCE_MS = envInt('IA_FLOW_WEBHOOK_DEBOUNCE_MS', 1_500)
...
export const DEFAULT_WEBHOOK_FALLBACK_MS = envInt('IA_FLOW_WEBHOOK_FALLBACK_MS', 15 * 60_000)
...
//   · on a slow fallback interval so a dropped delivery isn't fatal.
//
...
export class WebhookIssueManager extends SourceIssueManager {
  ...
  constructor(
    ...
    private readonly debounceMs: number = DEFAULT_WEBHOOK_DEBOUNCE_MS,
    private readonly fallbackMs: number = DEFAULT_WEBHOOK_FALLBACK_MS,
  ) {
  ...
  start(dispatch: (item: IssueItem) => Promise<void>): Disposable {
    ...
    log.info(
      { projectId: this.projectId, debounceMs: this.debounceMs, fallbackMs: this.fallbackMs },
  ...
  /** Queue a scan. Debounced so an event burst produces one cycle. */
