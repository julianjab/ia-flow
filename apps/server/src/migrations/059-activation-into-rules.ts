...
/** El Record legacy (`{ field: raw }`) → `WhenCondition[]`, un condition por
 *  entrada. La regla nace con una sola forma, y así el editor no tiene que
 *  soportar las dos. */
function legacyRecordToConditions(existing: Record<string, string>): WhenCondition[] {
...
/** El `logic` de la primera condición no significa nada (es el conector con
 *  la anterior); si el status quedó adelante, hay que correrlo. Muta en lugar. */
function fixLogicConnectors(conditions: WhenCondition[]): void {
...
/** Normaliza `when_conditions` (array o el Record legacy) a la lista de
 *  `WhenCondition[]` que va a la regla, con el status como primera condición. */
function buildConditions(
  a: AgentRow,
  existing: WhenCondition[] | Record<string, string> | null,
): WhenCondition[] {
  const conditions: WhenCondition[] = []
  if (a.status_name) conditions.push({ field: 'status', op: '=', value: a.status_name })
  if (Array.isArray(existing)) conditions.push(...existing)
  else if (existing) conditions.push(...legacyRecordToConditions(existing))
  fixLogicConnectors(conditions)
  return conditions
}

function migrateAgentActivation(
  ...
  if (!a.status_name && !hasWhen) return

  const conditions = buildConditions(a, existing)
...
const migration: Migration = {
  ...
  up(db) {
    ...
    for (const a of agents) migrateAgentActivation(db, a, now)
