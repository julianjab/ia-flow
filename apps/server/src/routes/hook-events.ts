...
type HookEventData = z.infer<typeof HookEventSchema>
...
export function createHookEventsRouter() {
  ...
  app.post('/', async (c) => {
    ...
    handleHookEvent(log, parsed.data)
...
function handleToolCall(log: ReturnType<typeof createLogger>, data: HookEventData): void {
  const { runId, toolName, toolUseId, parentToolUseId, input, result, isError, transcriptPath } =
    data
  log.info(
  ...
  if (result !== undefined) {
    log.info(
      { event: 'tool.result', runId, tool: toolName, toolUseId, parentToolUseId, result, isError },
      'Tool result',
    )
  }
...
function handleHookEvent(log: ReturnType<typeof createLogger>, data: HookEventData): void {
  const {
    runId,
    event,
    toolName,
  ...
  if (event === 'agent.stop' || event === 'subagent.stop') {
