...
/**
 * Por número de PR. No hay lookup inverso en el contrato —ninguna fuente
 * expone "dame el issue de este PR"—, así que se barre lo que el scan ya
 * tiene cacheado y se busca el PR entre los dev-links de cada item. Es la
 * misma lista que el dispatch usa en cada ciclo, así que en la práctica no
 * cuesta una llamada nueva.
 *
 * Sin `refresh`: si el board todavía no vio el link, el próximo scan lo va a
 * ver. Forzar un refetch acá pondría a cada delivery de CI —que llegan de a
 * decenas por push— a pagar una consulta completa del board.
 */
async function resolveByPrNumber(
  ...
  const candidates = repos?.length ? byNumber.filter((it) => touchesAnyRepo(it, repos)) : byNumber
...
export function createResolveEventItem(deps: ResolveEventItemDeps) {
  return async function resolveEventItem(
    ...
    if (scope.prNumber != null) {
      return resolveByPrNumber(source, projectId, scope.prNumber, scope.repos)
    }
