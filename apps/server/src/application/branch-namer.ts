...
function buildPrompt(task: BranchNamerTask): string {
  ...
  return [
    ...
    '',
    'Reglas estrictas:',
    '- Formato: <prefijo>/<slug-descriptivo>',
    '- Prefijo permitido: feat | fix | chore | refactor | docs (elegilo según tipo/descripción; functional → feat por defecto)',
    '- Slug: kebab-case, minúsculas, sin acentos, máximo 50 chars, describe la acción principal de forma concreta',
    '- Sin espacios, sin dobles guiones, sin puntos, sin comillas, sin markdown, sin sufijos numéricos ni IDs',
    '',
    'La branch se linkea al issue por API (no por nombre), así que NO incluyas el task ID ni el número de issue en el nombre. Foco en la acción.',
    '',
