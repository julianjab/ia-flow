...
type BucketRowSection = { kind: 'loose'; rows: OrderedTask[] };
/** Las filas de un bucket, en una única sección suelta — el `<ul>` de
 *  siempre. Antes cortaba `waiting-on-you` en sub-secciones por tema
 *  (propuestas del viejo "✦ Asistente", ver #216); esa feature se sacó del
 *  todo y no queda ningún camino que produzca un agrupamiento no-trivial. */
function bucketSections(bucket: { rows: OrderedTask[] }): BucketRowSection[] {
 ...
 *  TODA columna con etiqueta es arrastrable, con handle en su propio borde
 *  derecho: una columna sin handle se lee como una rota, no como "flexible a
 *  propósito" — de ahí que "issue" haya sido, en dos rondas de feedback
 *  seguidas, "el resize que falta". El track final (`spacer`) es el que
 *  absorbe lo que sobra: no tiene celda en el DOM (un track de más en
 *  `grid-template-columns` no necesita ítem — queda vacío), así que no hay
 *  ninguna columna VISIBLE sin su propio control. Mismo orden que el DOM de
  ...
  <section class="settings-section settings-section--list">
    ...
    <div class="task-table">
      ...
      <div class="task-thead">
        ...
        <span class="task-th-col">
          <span class="task-th-label" aria-hidden="true">issue</span>
...
.task-table {
  ...
  /* `clip` y NO `hidden`/`auto`, y la diferencia es funcional.
     `overflow: hidden`/`auto` convierten a la caja en un contenedor de
     scroll, y un `position: sticky` de adentro pasa a anclarse a ELLA en vez
     de a `.tk-list` (que es quien de verdad scrollea, ver ahí). `clip`
     recorta igual (que es lo único que se quería, para el radio) pero NO
     crea contenedor de scroll, así que el sticky sigue mirando a `.tk-list`.
     ...
     investigarlo con acceso al caso real — columnas muy anchas vuelven a
     recortarse sin scroll, que es el comportamiento anterior conocido. */
  overflow: clip;
}
.task-list {
  list-style: none;
